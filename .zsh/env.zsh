# Loaded from ~/.zshenv, so it runs for every zsh, interactive or not, and
# scripts and GUI programs started from a shell see the same environment.
# Nothing has to happen before it. Keep it cheap: Veil gives `zsh -l -c env`
# five seconds before falling back to launchd's bare PATH.
